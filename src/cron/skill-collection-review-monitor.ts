...
type ExplicitToolAllowList = { allow?: string[]; deny?: string[] };

/**
 * The review turn runs with `SKILL_WORKSHOP_MAINTENANCE_TOOLS`, intersected
 * with every explicit allowlist the run resolves. An agent whose allowlist
 * admits none of those tools can never run the review — the turn dies with
 * "No callable tools remain after resolving explicit tool allowlist" before it
 * starts — so no monitor is declared for it. Layers without an explicit
 * allowlist do not restrict the turn and are ignored.
 */
function allowsWorkshopMaintenance(cfg: OpenClawConfig, agentId: string): boolean {
  ...
  return SKILL_WORKSHOP_MAINTENANCE_TOOLS.some((tool) =>
    layers.every((layer) => isToolAllowedByPolicyName(tool, layer)),
  );
}
...
/** One system-owned review job per configured agent that can run the review, and its Workshop directory. */
export function resolveSkillCollectionReviewMonitorSpecs(
  ...
  return listAgentIds(cfg)
    .filter((agentId) => allowsWorkshopMaintenance(cfg, agentId))
    .map((agentId) => ({
      agentId,
      input: {
