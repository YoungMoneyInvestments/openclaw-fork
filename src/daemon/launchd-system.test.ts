...
describe("system LaunchDaemon ownership", () => {
  ...
  it("detects a readable owner after an unreadable foreign plist", async () => {
    ...
    state.plutilLabels.set(owner, "ai.openclaw.gateway");
  ...
  it("skips a valid plist without a Label key and detects a later owner", async () => {
    ...
    state.plutilNoLabelKeys.add(unrelated);
    state.files.set(owner, "<plist/>");
    state.plutilLabels.set(owner, "ai.openclaw.gateway");
